...
int good_suffix_rule_fun(int i, int* big_l_array, int len_big_l_array,
 ...
 int indet_good_suffix(int* pattern, int len_p, int* text, int i, int j){
    int* q_prime;
    //|q_prime| = |matched_substring_in_text| + |prefix_of_P of length MAX(m-j,j)| = j + MAX(m-j,j)

    q_prime = (int*)calloc(2*(j + len_p - 1), sizeof(int));
    //q_prime = (int*)calloc(2*MAX(len_p-j, j), sizeof(int));
    if ( i == 106){
        //printf("the value of J is = %d\n", j);
    }
    for(int l = 1; l <= j; l++){
        q_prime[l-1] = text[i + j - l];
        /*
        if(i == 106){
            printf("q_prime[%d] = %d\n", l-1, q_prime[l-1]);
        }
    ...
    if(j  != len_p){
        //for(int l = MAX(len_p-j, j)-1; l >= 0; l--){
        for(int l = len_p - 1; l >= 0; l--){
            q_prime[l] = pattern[l];
        }
    }
    
    prefixarray(q_prime, len_p);
...
int boyer_moore_fun(int* p, int len_p, boyer_moore p_bm, int* t, int len_t, int* occurrences, int sigma)
{
    ...
    while(i < len_t - len_p + 1){
        ...
        for(int j = len_p - 1; j > -1; j--){
            ...
            if(gcd(p[j], t[i + j]) == 1){
                int skip_bc = bad_character_rule_fun(j, t[i + j], p_bm.amap_s, p_bm.len_amap_s, p_bm.bad_char_s, p_bm.len_bad_char_s);
                int skip_gs = 0;
                if(indet(t[i + j], sigma) || len_p - j - 1 >= m_ell_suf){
                    //if(indet(t[i + j], sigma) || j >= m_ell_suf){
                    //printf("wrong indet");
        ...
        i += shift;
        //printf("IN boyer_moore_fun shift = %d\n", shift);
    }
    /*
...
int main(int argc, char *argv[])
{
    ...
    //printf("Total Brute force time =  %.7f sec\n", BF_time);
    printf("%.7f \n", BF_time);
    //printf("-------KMP Indet-------\n");
    start_KMP = clock();
    ...
    //printf("Total KMP_Time time =  %.7f sec\n", KMP_time);
    printf("%.7f \n", KMP_time);
    ...
    //printf("Total Boyer Moore time = %.7f sec\n", BM_time);
    printf("%.7f \n", BM_time);
    //printf("IT is runing\n");
    //printf("res is: %d", res);
    /*
